...
async function generateContentWithRetry(ai: GoogleGenAI, params: any, retries = 3, baseDelay = 2000) {
  ...
  const finalMsg = lastError?.message || lastError?.error?.message || "AI 服務暫時無法使用，請稍後再試 (Service Unavailable)。";
...
function getProTimeoutMs(prompt: string): number {
  ...
  if (len > 4000) return 50000;
  if (len > 2000) return 35000;
  return 20000;
}
 ...
 * ThinkingLevel.MINIMAL 避免 thought signature 複雜度，rawParts 封裝於 service 內部。
 */
