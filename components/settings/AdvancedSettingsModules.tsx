...
export const ProfileProgressWidget: React.FC = () => {
  const { user: profileUser, isLoading } = useUserProfileManager();
  const score = profileUser.profileCompleteness ?? 0;

  if (isLoading) {
    return (
      <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-gray-700 animate-pulse">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <div className="h-12 w-12 bg-slate-200 dark:bg-gray-600 rounded-lg" />
            <div>
              <div className="h-5 w-24 bg-slate-200 dark:bg-gray-600 rounded mb-2" />
              <div className="h-4 w-16 bg-slate-100 dark:bg-gray-700 rounded" />
            </div>
          </div>
          <div className="h-8 w-12 bg-slate-200 dark:bg-gray-600 rounded" />
        </div>
        <div className="w-full bg-slate-100 dark:bg-gray-700 rounded-full h-3 mb-4" />
        <div className="h-20 bg-slate-50 dark:bg-gray-700 rounded-xl" />
      </div>
    );
  }
...
export const NotificationSettingsGroup: React.FC = () => {
  const { user, prefLoadingKey, updateNotificationPreference } = useUserProfileManager();

  const handleToggle = (key: 'pushEnabled' | 'emailEnabled') => {
    if (navigator.vibrate) navigator.vibrate(50);
    const next = !user.preferences[key];
    updateNotificationPreference(key, next);
  };
  ...
  const Toggle = ({ checked, onClick, disabled }: { checked: boolean; onClick: () => void; disabled?: boolean }) => (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className={`relative w-12 h-7 rounded-full transition-colors duration-300 focus:outline-none flex-shrink-0 ${
        checked ? 'bg-indigo-600' : 'bg-slate-200 dark:bg-gray-600'
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-gray-700">
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <div>
              <h4 className="font-bold text-slate-700 dark:text-slate-200">推播通知</h4>
              <p className="text-slate-400 text-xs">接收 App 內即時推播訊息</p>
            </div>
            <Toggle
              checked={user.preferences.pushEnabled}
              onClick={() => handleToggle('pushEnabled')}
              disabled={prefLoadingKey === 'pushEnabled'}
            />
          </div>
          ...
          <div className="pt-4 border-t border-slate-100 dark:border-gray-700">
            <div className="flex items-center justify-between">
              <div>
                <h4 className="font-bold text-slate-700 dark:text-slate-200">電子郵件通知</h4>
                <p className="text-slate-400 text-xs">重要更新與批改報告寄送至信箱</p>
              </div>
              <Toggle
                checked={user.preferences.emailEnabled}
                onClick={() => handleToggle('emailEnabled')}
                disabled={prefLoadingKey === 'emailEnabled'}
              />
            </div>
          </div>
        </div>
